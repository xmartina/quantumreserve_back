...
$('#depositModalForm').on('submit', function(e){
    e.preventDefault();
    var url = e.currentTarget.action;
    var formid = '#' + $(this).attr('id');
    var prev_state_lang = $(this).attr('data-curr-lang');
    var load_state_lang = $(this).attr('data-load-lang');
    var modal = $(this).attr('data-modal');
    var reload = $(this).attr('data-reload');
    $(this).find(':input[type=submit]').prop('disabled', true);
    $(this).find(':input[type=submit]').html(load_state_lang);
    $('.error').hide();
    ...
    $.ajax({
        ...
        {  
            ...
            $("input[name="+res.csrfTokenName+"]").val(res.csrfHash);

            if(res.success == false)
            {
                $.each(res.errors, function(key, value){
                    var msg = '<label class="error" for="' + key + '">' + value +'</label>';
                    $('input[name="' + key + '"], select[name="' + key + '"]').addClass('is-invalid').after(msg);
                });
            } else {
                $('#' + modal).modal('hide');
                $(formid)[0].reset();
            ...
            $(formid).find(':input[type=submit]').html(prev_state_lang);
        },error: function(data){}
    })
})
